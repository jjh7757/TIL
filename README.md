...
- [공용 DB 서버 구성 — PostgreSQL + Redis](AWS배포환경/DB환경구성.md) — 앱마다 DB 컨테이너를 새로 띄우지 않고 하나를 공유하는 구조 설계와 실제 적용 기록. 원티드 채용공고 API 집계로 DB 선택 근거를 데이터로 남기고, RAM 2GB 예산에 DB 상주 비용을 반영, AWS CLI+SSM으로 직접 서버에 적용하며 겪은 트러블슈팅 포함
