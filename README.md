...
| 2026-08-18 | [파이썬 기초 — 스코프](CS지식/파이썬기초/13_스코프.md) — 함수 내부/외부 변수 접근 범위, LEGB 규칙(Local·Enclosed·Global·Built-in), enclosed 스코프와 중첩 함수, `global` 키워드로 외부 값을 바꾸는 예외 상황과 실무에서 지양해야 하는 이유, 파이썬은 if/for에 block scope가 없다는 점 정리 |
