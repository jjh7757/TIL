...
| 2026-08-14 | [왜샀어 Docker 배포 전환 작업 로그](vercel실습/왜샀어/Docker배포전환.md) — Vercel에서 네이버클라우드 Micro Server(Docker 셀프호스팅)로 옮기는 실제 작업 기록. 서버 업체 재선정(오라클 홈 리전 이슈), SSH·sshd 트러블슈팅, 스왑·Docker·ufw 구성, 도메인 연결과 Caddy HTTPS 발급, OAuth 리다이렉트 URI 등록까지 (진행 중) |
