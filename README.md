...
| 2026-09-14 | [LangGraph Plan-and-Execute](CS지식/LangGraph_PlanAndExecute.md) — ReAct(매 순간 다음 행동 결정) vs Plan-and-Execute(전체 계획 선수립) 구분과 적합한 작업 유형, `plan`은 덮어쓰기·`past_steps`는 `operator.add` 누적으로 필드마다 다르게 설계하는 이유, Replanner 출력을 `Union[Plan, Response]`로 강제해 "재계획 또는 최종 응답 중 하나만" 표현하기, 완료된 단계를 계획에서 안 빼면 무한 반복되는 문제를 프롬프트+`recursion_limit` 이중 장치로 막기, Replanner 없이도 되는 경우(재계획이 필요 없는 작업)와 그 실습 |
| 2026-09-15 | [LangGraph Human-in-the-Loop](CS지식/LangGraph_HumanInTheLoop.md) — `interrupt()`로 노드/Tool 내부에서 그래프를 통째로 멈추고 Checkpointer가 state를 기록해두는 원리, 그래프 state·interrupt payload·resume value 세 값의 역할, 재개 시 Tool 함수가 처음부터 다시 실행되지만 `interrupt()`는 다시 멈추지 않고 resume 값을 반환한다는 함정(그래서 interrupt 이전 코드는 재실행돼도 안전해야 함), 서버가 승인 대기 중 요청 하나를 붙들지 않고 `POST /chat` → `POST /resume` 두 번의 독립 요청으로 나누는 구조, 게시글 발행 승인·계획 재검토·부족한 정보 되묻기 실습 |
| 2026-09-16 | [LangGraph Agentic RAG](CS지식/LangGraph_AgenticRAG.md) — 검색 여부만 판단하는 RAG Agent에서 한발 더 나가 검색 결과의 근거 충분성까지 평가·재검색하는 구조, 직접 설계한 질문 분해+Send 동적 팬아웃+부족분 겨냥 재검색 조합을 사내 규정 19개·평가문항 50개로 실행(recall 97.2%), "문서는 찾았는데 grade는 부족 판정"으로 끝난 유일한 보류 사례가 보여준 문서 단위 평가의 한계(청크 단위로는 놓칠 수 있음), Send 기반 동적 팬아웃이 그래프 시각화에는 안 잡히는 함정과 해결법, [LangGraph Time Travel](CS지식/LangGraph_TimeTravel.md) — Checkpoint 조회·Replay(과거 값 그대로 재실행)·Fork(과거 값 수정 후 새 경로) 구분, thread_id와 checkpoint_id의 층위 차이, `parent_config`로 갈라지는 실행 경로 추적, State는 되감아도 이미 보낸 메일·결제 같은 외부 작업은 안 되돌아간다는 주의점, "다시 생성"·"수정 후 계속" 같은 UI 기능이 내부적으로 Replay/Fork로 매핑되는 표 |
| 2026-09-17 | [LangGraph Subgraph](CS지식/LangGraph_Subgraph.md) — 별도 그래프를 부모 그래프의 노드로 등록해 재사용하는 방법, 같은 State 스키마면 컴파일된 그래프를 그대로 `add_node`, 다른 스키마면 래퍼 함수로 입출력 변환, `subgraphs=True`로 내부 노드까지 스트리밍, [LangGraph Supervisor](CS지식/LangGraph_Supervisor.md) — Multi-Agent를 쓰는 이유(역할·Tool·Context 분리, 동적 위임), Subagent를 `@tool`로 감싸 중앙 Supervisor의 Tool로 등록하기, Checkpointer는 최상위 Supervisor에만 두는 이유, [LangGraph Handoff](CS지식/LangGraph_Handoff.md) — `Command(update=..., goto=...)`로 State 변경과 다음 노드 이동을 함께 결정하기, `graph=Command.PARENT`로 부모 그래프 노드로 이동하는 Handoff Tool, 담당 Agent가 바뀐 뒤 재지정 없이도 다음 턴부터 새 Agent가 이어받는 구조, Router·Supervisor·Handoff 세 패턴 비교 |
| 2026-09-18 | [LangGraph 멀티 에이전트 종합 실습](CS지식/LangGraph_멀티에이전트실습.md) — 조사·기획·검토 세 에이전트를 Router+조건부 엣지로 동적 조합(고정 순서가 있으면 Supervisor보다 예측 가능), 다른 State 스키마의 서브그래프/에이전트를 래퍼 노드로 입출력 변환해 연결, "승인" 부분 문자열 판정의 오탐(마지막 줄 정확 일치로 해결), 세션 상태 재사용에서 겪은 반대 방향 두 버그(낡은 상태 무조건 재사용 vs 이번 턴 실행 여부로만 판단)를 Router의 `is_followup` 구조화 출력으로 해결, 같은 함정이 화면 출력 로직에도 있었다는 점, `draw_mermaid_png()`로 실제 컴파일된 그래프 구조 검증 |
...
- [요구사항 명세서 작성법](프로그램방법론/요구사항명세서작성법.md) — 기준선 다음 단계. 검증 가능한 요구사항 문장 규칙, ID 체계, 인수 조건 3줄(정상·예외·경계), 비기능 8항목, MoSCoW Must 60% 규칙, 추적표, LLM 기능 요구사항 쓰는 법
- [요구사항 명세서 템플릿](프로그램방법론/요구사항명세서템플릿.md) — 서비스마다 복사해서 채우는 빈 양식(개요·범위·사용자 스토리·기능/비기능 요구사항·우선순위·가정/제약/미결·추적표·완료 체크리스트·변경 기록)
...
- [LangGraph Plan-and-Execute](CS지식/LangGraph_PlanAndExecute.md) — ReAct vs Plan-and-Execute 구분, 필드별 reducer 설계(`plan` 덮어쓰기 vs `past_steps` 누적), `Union[Plan, Response]`로 Replanner 출력 강제, `recursion_limit`으로 무한 반복 방지
- [LangGraph Human-in-the-Loop](CS지식/LangGraph_HumanInTheLoop.md) — `interrupt()`로 그래프 중단, state·payload·resume 세 값의 역할, 재개 시 재실행 함정, `POST /chat`+`POST /resume` 두 요청으로 나누는 서버 구조
- [LangGraph Agentic RAG](CS지식/LangGraph_AgenticRAG.md) — 검색 결과 근거 평가·재검색 구조, 질문 분해+Send 동적 팬아웃+부족분 겨냥 재검색을 직접 설계해 50문항 평가(recall 97.2%), 문서 단위 평가의 한계, Send 팬아웃의 그래프 시각화 함정
- [LangGraph Time Travel](CS지식/LangGraph_TimeTravel.md) — Checkpoint 조회·Replay·Fork 구분, thread_id vs checkpoint_id, `parent_config`로 갈라지는 실행 경로, 외부 작업은 안 되돌아간다는 주의점
- [LangGraph Subgraph](CS지식/LangGraph_Subgraph.md) — 별도 그래프를 부모 그래프의 노드로 등록해 재사용, 같은/다른 State 스키마에 따른 등록 방식(그대로 vs 래퍼 함수), `subgraphs=True`로 내부 노드까지 스트리밍
- [LangGraph Supervisor](CS지식/LangGraph_Supervisor.md) — Multi-Agent를 쓰는 이유, Subagent를 `@tool`로 감싸 중앙 Supervisor의 Tool로 등록, Checkpointer는 최상위 Supervisor에만 설정
- [LangGraph Handoff](CS지식/LangGraph_Handoff.md) — `Command(update=..., goto=...)`로 State 변경과 노드 이동을 함께 결정, `graph=Command.PARENT`로 부모 그래프 노드로 이동하는 Handoff Tool, 담당 Agent 전환이 다음 턴까지 유지되는 구조, Router·Supervisor·Handoff 비교
- [LangGraph 장애 허용](CS지식/LangGraph_장애허용.md) — `RetryPolicy`로 노드 단위 자동 재시도, super-step별 checkpoint, `invoke(None, config)`로 실패 지점부터 재개, 병렬 분기 일부 실패 시 성공분 보존, `TimeoutPolicy`·`error_handler`·멱등성 키로 중복 부수효과 방지
- [LangGraph Agent Tool 실행 검증과 통제](CS지식/LangGraph_ToolControl.md) — `wrap_tool_call`/`wrap_model_call` Middleware, LLM Tool 인자 vs 프로그램 실행 Context 분리, 실행 전(인자·권한·승인)·실행 후(반환값) 검증, `ToolCallLimitMiddleware`로 호출 횟수 제한
- [LangGraph 로깅](CS지식/LangGraph_로깅.md) — 로거·핸들러·포매터 구조, 로그 레벨 5단계, 로거·핸들러 2단계 필터링, `RotatingFileHandler`, `logger.exception()`으로 traceback 기록
- [LangGraph 멀티 에이전트 종합 실습](CS지식/LangGraph_멀티에이전트실습.md) — Router+조건부 엣지로 조사·기획·검토 동적 조합, 다른 State 스키마의 서브그래프/에이전트를 래퍼 노드로 연결, "승인" 부분 문자열 판정의 오탐과 마지막 줄 정확 일치로의 수정, 세션 상태 재사용에서 겪은 반대 방향 두 버그를 Router의 `is_followup` 판단으로 해결, 화면 출력 로직의 같은 함정, `draw_mermaid_png()`로 그래프 구조 검증
