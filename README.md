...
- [모니터링 / 알림](AWS배포환경/모니터링구성.md) — RAM 제약상 상주 모니터링 컨테이너 대신 cron 5분 간격 스크립트 + CloudWatch SNS 이메일 알림으로 구성. 외부 서비스로는 볼 수 없는(외부 미노출) DB 상태까지 서버 내부에서 확인, 상태 전이 시에만 알려 스팸 방지, 컨테이너를 실제로 중지·재시작해 알림 파이프라인 검증
