...
> **정정**: 아래 MRR 비교에는 측정 비대칭이 있다. `EnsembleRetriever`의 반환값을 `[:k]`로 자르지 않아 **Hybrid만 최대 10개까지 훑고 나머지 전략은 5개까지만 훑었다.** `reciprocal_rank()`가 리스트 전체를 스캔하므로 Hybrid의 MRR이 구조적으로 과대평가됐고, 관측된 우위 폭(0.002~0.005)이 그 편향 크기와 같은 자리수다. Hit@k 계열은 `docs[:k]`로 자르고 들어가므로 영향이 없다. 자세한 검증은 [RAG 실험 스크립트 비교분석](RAG_실험스크립트_비교분석.md)에 정리했다.
...
- [RAG 실험 스크립트 비교분석](RAG_실험스크립트_비교분석.md) — 같은 과제의 참고 구현과 input/output 목록 대조, 위 MRR 결론의 측정 비대칭 검증
